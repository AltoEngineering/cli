 ...
 */
Alto.genModel = module.exports = Alto;

Alto.genModel = Alto.Object.create({
    ...
    _templateDirectoryPath: 'templates/model',

    _templateFilePath: 'templates/model/model_template.js',
    ...
    action: function (args) {
        if (!args[0]) {
            console.log(clc.red('alto model requires a model name.'));
            Alto.cli.displayOptions()
            return
        }

        this.set("modelName", args[0]);
    ...
    cloneTemplate: function () {
        var fileTemplate = fs.createReadStream(this.get('_fullTemplateFilePath'))
            ...
            , file = file.replace(/<model_name>/gi, s(this.get('modelName')).capitalize().s + 'Record');

        this._writeFile(file);
    },

    _writeFile: function (file) {
        wrench.mkdirSyncRecursive('app/models/', 0777);

        if (fs.existsSync('app/models/' + this.get('modelName') + '_model.js')) {
            console.log(clc.red('A file named ' + this.get('modelName') + '_model.js' + ' already exists.  ' +
            'Canceling model creation.'));
        } else {
            fs.writeFileSync('app/models/' + this.get('modelName') + '_model.js', file)
        }

 ...
 */
Alto.genController = module.exports = Alto;


Alto.genController = Alto.Object.create ({
    ...
    _templateDirectoryPath: 'templates/controller',

    _templateFilePath: 'templates/controller/controller_template.js',
    ...
    action: function (args) {
        if (!args[0]) {
            console.log(clc.red('alto controller requires a controller name.'));
            Alto.cli.displayOptions()
            return
        }

        this.set("controllerName", args[0]);
    ...
    cloneTemplate: function () {
        var fileTemplate = fs.createReadStream(this.get('_fullTemplateFilePath'))
            ...
            , file = file.replace(/<controller_name>/gi, this.get('controllerName') + 'Controller');

        this._writeFile(file);
    },

    _writeFile: function (file) {
        wrench.mkdirSyncRecursive('app/controllers/', 0777);

        if (fs.existsSync('app/controllers/' + this.get('controllerName') + '_controller.js')) {
            console.log(clc.red('A file named ' + this.get('controllerName') + '_controller.js' + ' already exists.  ' +
            'Canceling controller creation.'));
        } else {
            fs.writeFileSync('app/controllers/' + this.get('controllerName') + '_controller.js', file)
        }
